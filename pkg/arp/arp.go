...
func rxHandler(dev *net.Device, data []byte, src, dst net.HardwareAddress) error {
	...
	merge := repo.update(msg.sourceProtocolAddress, msg.sourceHardwareAddress)
...
func reply(iface net.ProtocolInterface, targetProtocolAddress, targetHardwareAddress []byte) error {
	dev := iface.Device()
	hdr := header{
...
func Resolve(iface net.ProtocolInterface, target, data []byte) ([]byte, error) {
	repo.mutex.RLock()
